...
function generateHeader(doc, {
    ...
    doc
        ...
        .text(`Subject: E-Ticket ID ${ticketId}`, 50, 75, { align: 'left' })
        .text(`Date: ${dateSend}`, 200, 57, { align: 'right' })
        .text(`To: ${toEmail}`, 200, 75, { align: 'right' })
        // [END] HEADER
        .strokeColor('#A6A6A6')
        .lineWidth(1)
        .moveTo(50, 100)
        ...
        .text(`ID ${ticketId}`, 50, 155, { align: 'right' })
        .lineWidth(1)
        .moveTo(50, 200)
        .lineTo(555, 200)
        .stroke()
        ...
        .fill("black")
        .font('poppins-bold')
        .text(`${busType}`, 50, 250, { align: 'left' })
        .fill("red")
        .text(`Seat No: ${seatNo}`, 50, 250, { align: 'right' })
        .fill("black")
        .font('poppins-regular')
        // origin
        .text(`${originDate}`, 50, 290, { align: 'left' })
        .font('poppins-bold')
        .text(`${originTime}`, 50, 305, { align: 'left' })
        .font('poppins-regular')
        .text(`${originCity}`, 50, 320, { align: 'left' })
        .text(`${duration}`, 50, 290, { align: 'center' })
        .text(`${kilometer}`, 50, 312, { align: 'center' })
        .moveTo(startX, yPosition)
        .lineTo(startX + lineLength, yPosition)
        .stroke()
        // destination
        .text(`${destinationDate}`, 50, 290, { align: 'right' })
        .font('poppins-bold')
        .text(`${destinationTime}`, 50, 305, { align: 'right' })
        .font('poppins-regular')
        .text(`${destinationCity}`, 50, 320, { align: 'right' })
        .lineWidth(1)
        .moveTo(50, 350)
        .lineTo(555, 350)
        .stroke()
        ...
        .font('poppins-regular')
        .text(`${passenger?.username || ""}`, 150, 400,)
        .text(`${passenger?.mobile || ""}`, 150, 420,)
        .text(`${passenger?.email || ""}`, 150, 440,)
        .text(`${passenger?.pickup || ""}`, 150, 460,)
        .fontSize(18)
        .fill("black")
        .fill("#0057A8")
        .font('poppins-bold')
